...
namespace rt {
    ...
    Material::SampleReflectance DielectricMaterial::getSampleReflectance(const Point& texPoint, const Vector& normal, const Vector& outDir) const {
        ...
        std::tie(rParallel, rPerp) = dielectricFresnel(_eta_i, _eta_t, std::abs(cos), std::abs(cos_t));
        ...
        if (sin > (_eta_t / _eta_i)) {
            ...
            return SampleReflectance(refracted.normalize(), RGBColor::rep(rt::sqr( _eta)));
        }
